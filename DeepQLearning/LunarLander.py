...
import matplotlib.pyplot as plt
from DeepQlearning_StableBaseline import DeepQlearningBase
...
def plot_reward_per_episode(rewards ,fileName,rel_ep,best_score):
    plt.scatter(rel_ep,best_score,c='red')    
    plt.plot(rewards)
    plt.xlabel('Episodes')
    plt.ylabel('# Rewards')
...
def main(args):
    ...
    if(args.train):
        ...
        if(args.algorithm == 1):
            ...
            print("\n > Utilizando o DeepQLearning implementado \n")
            
            DQN =   DeepQLearning(env, gamma, epsilon, epsilon_min, epsilon_dec, episodes, batch_size, memory, model,max_steps)
            rewards,best_model,relative_episode,best_rewards = DQN.train()
            # Grafico de rewards
            plot_reward_per_episode(rewards, plot_rewards_file,relative_episode,best_rewards)
            best_model.save(model_filename)

        elif(args.algorithm == 0):
            
            print("\n > Utilizando o DeepQLearning de um módulo python \n") 
            DQN = DeepQlearningBase()
            DQN.train()
            DQN.save_model('model_stable/model')
            DQN.execute()
            DQN.plot()
    else:
